...
Для меня этот риск вполне конкретен. Я целиком нахожусь в экосистеме Apple. Mac и iPhone выглядят как два отдельных устройства, но оба завязаны на инфраструктуру одного поставщика. Если доступ к ней будет ограничен, значительная часть моей цифровой среды может стать недоступна сразу на обоих устройствах. На самом деле именно осознание этого факта и побудило меня заняться, наконец, вопросами цифровой устойчивости.
...
1. **Найти корни в системе зависимостей.** Корень — это аккаунт, через который восстанавливаются другие сервисы и у которого есть хотя бы один последний способ восстановления, не зависящий от нижестоящих аккаунтов и того же контура отказа.
...
При этом сторонние сервисы я исторически завязывал не на Apple, а на Google. Объективного архитектурного преимущества здесь, вероятно, нет: оба аккаунта принадлежат крупным экосистемам и могут использоваться как корни для других сервисов. Просто Google воспринимается мной как более самостоятельный аккаунт, а Apple — как часть инфраструктуры устройств. Поэтому внешнее дерево выросло вокруг Google, а Apple обслуживает собственную экосистему.
...
В этом случае актуальная исходная версия данных находится внутри чужого сервиса. Например: контакты, задачи в менеджере задач, календарь, почта, документы в облачных редакторах, настройки сервисов.
...
Есть еще один особый класс данных: то, что не является содержимым процессов, но обеспечивает доступ ко всей цифровой инфраструктуре.
...
Часть ключевых секретов дополнительно остается на бумаге дома. Это позволяет начать восстановление, не имея доступа к менеджеру паролей или архиву секретов.
...
| Пароли и PIN-коды устройств                                      |                  |                | ✓           | ✓      |
| Пароль зашифрованного офлайн-накопителя                          |                  |                | ✓           | ✓      |
| Пароль архива секретов                                           |                  |                | ✓           | ✓      |
| Пароль менеджера паролей (если отдельный от основной экосистемы) |                  |                | ✓           | ✓      |

В ежедневной работе пароли берутся из менеджера, а TOTP-коды — из аутентификатора. Для восстановления экспорт менеджера вместе с TOTP-секретами и кодами восстановления хранится в зашифрованном архиве секретов, который входит в резервную копию. А пароли и коды, необходимые для запуска самого восстановления, вынесены за пределы этой системы — на бумагу и в память.
...
Такой пароль не требуется помнить или уметь заново выводить из какой-либо логики. Корневые пароли генерируются точно так же, но дополнительно сохраняются в архиве секретов и на бумаге: хранить пароль Apple только внутри экосистемы Apple, а пароль Google только в Google Password Manager было бы замкнутым кругом.
...
Для данных, которые хранятся локально, недостаточно увидеть файлы в резервной копии. Нужно хотя бы один раз восстановить выбранную папку в пустое место, и открыть файлы.
