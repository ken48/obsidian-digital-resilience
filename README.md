...
С точки зрения пользователя кажется, что теперь не используется второй фактор. Но технически второй фактор заменяется подтверждением владения ключом. Сервис проверяет, что пользователь действительно имеет доступ к хранилищу passkey, а пользователь подтверждает этот доступ кодом устройства или биометрией.
