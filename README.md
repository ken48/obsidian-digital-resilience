...
Дальше работа идет в этом порядке.
...
В российском контуре центральным узлом стал Яндекс. Здесь ситуация менее надежная: последняя точка восстановления находится не в секрете на физическом носителе, а в процедуре подтверждения личности через саппорт.
...
В этом случае данные уже находятся под моим контролем. Например: фотоархив, документы, исходники, заметки, прочие файлы. Про личный архив я уже писал отдельно в статье [«Личный архив: сбор, бэкап, таймлайн фотографий»](https://habr.com/ru/articles/1038164). Здесь задача понятная: обеспечить независимые копии, которые переживут поломку устройства, случайное удаление или физическую потерю.

Отдельный случай — данные, где важна история изменений. Например, исходный код, конфиги и заметки, которые хранятся в системах контроля версий. Там есть механизм зеркалирования.
...
Когда я это понял, я стал выбирать аутентификатор, у которого TOTP-секреты хранятся под моим контролем и есть экспорт-импорт. Я выбрал Ente Auth.
...
- TOTP-секреты;
- резервные коды;
- прочие данные для восстановления доступов.

Сам архив хранится независимо: в двух облачных хранилищах разных контуров и на офлайн накопителе.

Ключ к архиву — мастер-пароль — хранится отдельно от самого архива и записан на бумаге.

#### 4.2.4 Схема целиком

Все подготовленные копии я распределяю между несколькими независимыми местами хранения: двумя облачными хранилищами в разных контурах и одним офлайн-накопителем. При этом способ подготовки зависит от типа данных: локальные данные резервируются напрямую, данные из сервисов сначала экспортируются, а секреты доступов собираются в отдельный зашифрованный архив. Для синхронизации данных я использую `rclone`, который упоминал в [прошлой статье](https://habr.com/ru/articles/1038164).
...
Из этого следует, что задача не в том, чтобы убрать все точки отказа. Некоторые из них неизбежны. Важно знать, где они находятся, что именно они могут сломать и как восстановиться. Устойчивость начинается там, где катастрофа превращается в обозримый и посильный список задач.